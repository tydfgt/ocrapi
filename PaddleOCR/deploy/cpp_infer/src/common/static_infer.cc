...
absl::StatusOr<std::shared_ptr<paddle_infer::Predictor>> PaddleInfer::Create() {
  ...
  if (option_.DeviceType() == "gpu") {
    ...
    if (option_.RunMode() == "paddle_fp16" || option_.RunMode() == "paddle_trt_fp16") {
    ...
    if (option_.RunMode().find("paddle_trt") != std::string::npos) {
      ...
      paddle_infer::PrecisionType trt_precision =
          use_fp16 ? paddle_infer::PrecisionType::kHalf
                   : paddle_infer::PrecisionType::kFloat32;
      config.EnableTensorRtEngine(1 << 30,  // 1GB workspace
                                  -1,        // all batches
                                  2,         // min subgraph size
                                  trt_precision,
                                  true,      // use_static = cache engine
                                  false);    // use_calib_mode
      config.EnableTunedTensorRtDynamicShape();
      config.SetOptimizationLevel(3);
      INFOW("TensorRT engine enabled (FP16=%d, static_cache=true)", use_fp16);
    }
